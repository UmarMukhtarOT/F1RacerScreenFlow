...
public class GarageManager : MonoBehaviour
{
    ...
    [SerializeField]
    private List<GameObject> coreInUseContainers;
   
    [SerializeField]
    private List <GameObject> coresAvailableContainers;
    [Header("Cores")]
    [SerializeField]
    ...
    private void Awake()
    {
        currentSelectedVehicleIndex = PlayerPrefs.GetInt(PlayerPrefKeys.currentSelectedVehicleIndex,0);
    }
    ...
    private void UpdateVehicleStatOnCoreAdd(Core core)
    {
        ...
        Debug.Log("Adding values of core");
        ...
        {
            if (Vehicles[currentSelectedVehicleIndex].vehicleStats.speed + core.statValue <= core.maxValue)
            {
        ...
        {
            if (Vehicles[currentSelectedVehicleIndex].vehicleStats.acceleration + core.statValue <= core.maxValue)
            {
        ...
        {
            if (Vehicles[currentSelectedVehicleIndex].vehicleStats.boost + core.statValue <= core.maxValue)
            {
    ...
    private void UpdateVehicleStatOnCoreRemove(Core core)
    {
        ...
        Debug.Log("Removing values of core");

        if (core.coreType == CoreType.SPEED)
        {
            if (Vehicles[currentSelectedVehicleIndex].vehicleStats.speed>0)
            {
                Vehicles[currentSelectedVehicleIndex].vehicleStats.speed -= core.statValue;
            }
        ...
        if (core.coreType == CoreType.ACC)
        {
            if (Vehicles[currentSelectedVehicleIndex].vehicleStats.acceleration > 0)
                Vehicles[currentSelectedVehicleIndex].vehicleStats.acceleration -= core.statValue;
        }
        if (core.coreType == CoreType.BOOST)
        {
            if (Vehicles[currentSelectedVehicleIndex].vehicleStats.boost > 0)
                Vehicles[currentSelectedVehicleIndex].vehicleStats.boost -= core.statValue;
        }
        OnVehicleStatUpdate?.Invoke(Vehicles[currentSelectedVehicleIndex].vehicleStats);
        SaveVehicleStat();
    ...
    public void ShowNextVehicle()
    {
        ...
        LoadVehicleStat();
        LoadVehicleCores();
        PlayerPrefs.SetInt(PlayerPrefKeys.currentSelectedVehicleIndex, currentSelectedVehicleIndex);
        OnVehicleStatUpdate?.Invoke(Vehicles[currentSelectedVehicleIndex].vehicleStats);
    }
    ...
    {
        JsonUtility.FromJsonOverwrite(PlayerPrefs.GetString(Vehicles[currentSelectedVehicleIndex].vehicleStats.vehicleName),
            Vehicles[currentSelectedVehicleIndex].vehicleStats);
    }

    private bool isOnce = true;
    private void SaveVehicleCores()
    {
       for(int i = 0; i < coreInUseContainers.Count; i++)
       {
            if (coreInUseContainers[i].GetComponent<UIDropSlot>().currentItem)
            {
                PlayerPrefs.SetInt(Vehicles[currentSelectedVehicleIndex].vehicleStats.vehicleName + "_core" + i, 
                    coreInUseContainers[i].GetComponent<UIDropSlot>().currentItem.GetComponent<Core>().coreId);
    ...
    {
        for (int i = 0; i < coresAvailableContainers.Count; i++)
        {
            ...
            if (isFirstTimeGameLoad == 1)
            {
                if (coresAvailableContainers[i].GetComponent<UIDropSlot>().currentItem)
                {
                    Debug.Log("Save, Fill Amount changes 1 ");
    ...
    private void LoadAvailableCores()
    {
        for (int i = 0; i < coresAvailableContainers.Count; i++)
        {
            ...
            if (coreID != 0)
            {
                ...
                Debug.Log("Load, Fill Amount changes  2 ");
